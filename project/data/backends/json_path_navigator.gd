...
## @param default_value Optional default value to return if path is not found
## @return NavigationResult object containing the result
static func navigate(json_data: Variant, path: Array, default_value: Variant = null) -> NavigationResult:
	if json_data == null:
		...
		if current_data is Dictionary:
			if current_data.has(path_part):
				...
				return NavigationResult.new_not_found(
					...
					current_path,
					_get_available_keys(current_data)
				)
...
static func _create_result_for_type(data: Variant, path: Array) -> NavigationResult:
	if data is Dictionary:
		return NavigationResult.new_dictionary(data, path)
	elif data is Array:
		return NavigationResult.new_array(data, path)
	else:
...
static func get_value(json_data: Variant, path: Array, default_value: Variant = null) -> Variant:
	var result := navigate(json_data, path)
	if result.found:
		return result.value
...
static func get_int(json_data: Variant, path: Array, default_int: int = 0) -> int:
	...
	if result.found:
		...
		elif result.value is String and result.value.is_valid_int():
...
static func get_float(json_data: Variant, path: Array, default_float: float = 0.0) -> float:
	...
	if result.found:
		...
		elif result.value is String and result.value.is_valid_float():
			return result.value.to_float()
...
static func get_bool(json_data: Variant, path: Array, default_bool: bool = false) -> bool:
	...
	if result.found:
		if result.value is bool:
			...
			var lower_str = result.value.to_lower()
