...
func get_data(p_path: Array[Variant], p_key: String) -> Variant:
	...
	Log.debug(
		"Navigating path with JSONPathNavigator",
		{
	...
	if nav_result.found:
		...
		Log.info(
			"Successfully navigated to data",
			{
				"path": p_path,
				"key": p_key,
...
func _load_local_data() -> bool:
	Log.info(
		...
		{"file": current_file, "backend_id": get_instance_id()},
	...
	Log.debug(
		"File read complete",
		{"file": current_file, "size_bytes": json_size, "backend_id": get_instance_id()},
		[Log.TAG_LOCAL]
	)
	...
	if json_result == null:
		Log.error(
			"Failed to parse local data JSON",
			{"file": current_file, "backend_id": get_instance_id()},
			[Log.TAG_LOCAL, Log.TAG_ERROR]
		)
		return false
	...
	if json_result is Dictionary:
		...
		Log.error(
			...
			{
				"file": current_file,
				"result_type": typeof(json_result),
				"backend_id": get_instance_id()
			},
			[Log.TAG_LOCAL, Log.TAG_ERROR]
		)
		...
		for key_name: String in top_level_keys:
			if key_name is String:
				...
				if value_item is Dictionary:
					...
					structure_info[key_name] = {"type": typeof(value_item)}

		Log.debug(
			"Data structure details",
			{"structure": structure_info, "backend_id": get_instance_id()},
			[Log.TAG_LOCAL, Log.TAG_DB]
		)
