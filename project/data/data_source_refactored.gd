...
func activate_card_cache() -> void:
	...
	if not cards:
		Log.error("Cannot activate card cache - cards collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return
...
func get_all_cards(use_cache: bool = false) -> Array[Dictionary]:
	if not cards:
		Log.error("Cannot get all cards - cards collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return []
...
func get_all_items() -> Array:
	if not items:
		Log.error("Cannot get all items - items collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return []

	var result: Array = await items.get_all()
	return result
...
func get_user_data(uuid: String = "") -> Dictionary:
	if not players:
		Log.error("Cannot get user data - players collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return {}
...
func get_event_data() -> Array:
	if not events:
		Log.error("Cannot get event data - events collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return []

	var result: Array = await events.get_all()
	return result
...
func get_event_lineups_data(event: String) -> Dictionary:
	if not events:
		Log.error("Cannot get event lineups - events collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return {}
...
func save_event_lineup_data(
	...
	if not events:
		Log.error("Cannot save event lineup - events collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return ""
...
func save_user_data(data: Dictionary) -> bool:
	if not players:
		Log.error("Cannot save user data - players collection not initialized", {
			"instance_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_ERROR])
		return false
...
func _log_active_collections() -> void:
	...
	Log.debug("Active collections", {
		...
		"backend_type": _backend.get_class() if _backend else "none",
		"instance_id": get_instance_id()
	}, [Log.TAG_DB])
