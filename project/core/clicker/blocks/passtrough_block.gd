...
static func deserialize_from_dict(data: Dictionary, game: Game = null) -> Block:
	...
	if not passtrough_block:
		Log.error("Failed to create passthrough block from factory", {}, ["serialization", "error"])
		return null
	...
	Log.debug(
		...
		{
			"object_type": passtrough_block.object_type,
			"block_context": passtrough_block.block_context
		},
