...
func setup_card(card: Card) -> void:
	...
	var id: String = info.card_definition.id
	...
	var card_name: String = info.card_definition.card_name
	card_info.set_card_name(card_name)
	card_info.set_card_level(info.level)
	card_info.add_tag("testtag1")
	card_info.add_tag("testtag1")
	var desc: String = info.card_definition.description
	card_info.set_rules_text(desc)
	card_info.set_upgrade_level(info.card_definition.upgrade_level)
