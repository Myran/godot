...
## Detect inner class name from object string representation
## Used for inner classes that don't have global names
func _detect_inner_class_name(obj: Object, scr: Script) -> String:
	# Use object's string representation to extract inner class name
	var obj_str = str(obj)
	
	# Extract class name from patterns like "<Context.Event#123>" or "<core.UpgradeEvent#456>"
	var regex = RegEx.new()
	regex.compile(r"<([^.]+)\.([^#>]+)")
	var result = regex.search(obj_str)
	if result:
		var full_name = result.get_string(1) + "." + result.get_string(2)
		return full_name
	
	# Fallback: try to extract from simple patterns like "<SomeClass#123>"
	regex.compile(r"<([A-Za-z][A-Za-z0-9_]*)")
	result = regex.search(obj_str)
	if result:
...
## Enhanced to support inner classes automatically.
func register_custom_class(scr: Script) -> PicklableClass:
	var clsname := scr.get_global_name()
	if clsname.is_empty():
		# Try to register inner classes from this script
		return _register_inner_classes_from_script(scr)
	
	return _register_class_with_name(scr, clsname)


## Register inner classes found in a script
func _register_inner_classes_from_script(scr: Script) -> PicklableClass:
	# For scripts with inner classes, we register them with a composite name
	var script_path = scr.resource_path
	var composite_name = StringName(script_path + "::inner_classes")
	
	# Create a basic registration for the script's inner classes
	var pc := PicklableClass.new()
	pc.constructor = scr.new  # This won't work for inner classes, but provides fallback
	
	# Note: Inner classes require special handling during object creation
	# They need to be instantiated through their parent script context
	
	class_registry[composite_name] = pc
	push_warning("Registered script with inner classes: " + script_path)
	return pc


## Register a class with a specific name - core registration logic
func _register_class_with_name(scr: Script, clsname: StringName) -> PicklableClass:
	var pc := PicklableClass.new()
