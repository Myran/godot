...
## Handles mouse down events for item selection
func _handle_mouse_down(position: Vector2) -> void:
	# Let the individual controllers handle this
	pass
...
## Get drag data implementation - delegates to the drag drop helper
...
func _on_config_changed(section: String, key: String, value: Variant) -> void:
	# Update internal state and UI based on config changes
...
## Saves format settings to the ConfigManager
...
func _apply_defaults() -> void:
	...
	# Reset tag lists
	_tag_list_controller.get_tag_lists().available_tags.clear()
	_tag_list_controller.get_tag_lists().active_tags.clear()
	_tag_list_controller.get_tag_lists().ignored_tags.clear()
