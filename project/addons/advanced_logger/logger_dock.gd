...
func _connect_signals() -> void:
	...
	var buffer_container := get_node_or_null("VBoxContainer/GridContainer/HBoxContainer") as HBoxContainer
	var retro_container := get_node_or_null("VBoxContainer/GridContainer/HBoxContainer2") as HBoxContainer
	...
	if buffer_container and retro_container:
		...
		if buffer_apply and retro_apply:
			buffer_apply.pressed.connect(func() -> void: _validate_and_apply_buffer_size(buffer_size_spin.value))
			retro_apply.pressed.connect(func() -> void: _validate_and_apply_retro_window(retro_spin.value))

	# Get tag management buttons
	var add_button := get_node_or_null("VBoxContainer/VBoxContainer/HBoxContainer/Button") as Button
	var remove_button := get_node_or_null("VBoxContainer/VBoxContainer/HBoxContainer/Button2") as Button
	var clear_button := get_node_or_null("VBoxContainer/VBoxContainer/HBoxContainer/Button3") as Button
