...
func _ready():
	print("debug ready")
	connect(SIGNAL_DEBUG,self,"_on_debug_event")
	for btn in v_box_container_buttons.get_children():
		btn.connect("pressed",self,"debug_button_pressed",[btn.name])
...
func debug_button_pressed(name):
	match name:
		...
		"button_pop_enemy":
			print("pop enemy")
			for n in 3:
				var new_card = card_controller.create_unit_from_id(n,1)
