class_name DamageShieldAbility extends Ability

var shield_used: bool = false


func deep_duplicate() -> Ability:
	var copy: DamageShieldAbility = DamageShieldAbility.new()
	copy.persistence_type = self.persistence_type
	copy.shield_used = self.shield_used
	return copy


func serialize_to_dict() -> Dictionary:
	var base_data: Dictionary = super.serialize_to_dict()
	base_data["shield_used"] = shield_used
	return base_data


func handle_battle_event(event: BattleAbilityEvent) -> void:
	if shield_used:
		return

	if event.phase == core.Tempus.PRE and event.event is BattleContext.DamageEvent:
		var damage_event: BattleContext.DamageEvent = event.event as BattleContext.DamageEvent
		var is_target_unit: bool = (
			damage_event.is_allied_side == event.is_allied
			and damage_event.target_position == event.position
		)
		if is_target_unit:
			damage_event.damage_effects.append({"effect_type": "shield", "ability": self})


func handle_draft_event(event: DraftAbilityEvent) -> void:
	if event.phase == core.Tempus.POST and event.event is core.BlockEntersPlay:
		var play_event: core.BlockEntersPlay = event.event as core.BlockEntersPlay
		if event.unit == play_event.block and not shield_used:
			var card: Card = event.unit as Card
			card.show_shield()
