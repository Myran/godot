...
func _init(new_seed: int = 0) -> void:
	...
	if new_seed == 0:
		new_seed = GameConstants.RandomSystem.DEFAULT_SEED
	reset(new_seed)


func reset(new_seed: int = 0, hard: bool = false) -> void:
	if hard:
		new_seed = GameConstants.RandomSystem.HARD_RESET_SEED
	if new_seed != 0:
...
func load_state(json_state: String) -> void:
	...
	if error == OK:
		...
		Log.error(
			...
			{"error": json.get_error_message(), "json": json_state, "line": json.get_error_line()},
			[Log.TAG_RNG, Log.TAG_ERROR]
		)
