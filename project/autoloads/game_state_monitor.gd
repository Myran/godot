...
func _find_game_node() -> Game:
	...
	var result: Node = _find_game_node_recursive(get_tree().root)
	if result and result is Game:
...
func _find_game_node_recursive(node: Node) -> Node:
	...
	if node and node is Game:
		return node
	
	for child in node.get_children():
		var result: Node = _find_game_node_recursive(child)
		if result:
			return result
...
func _process(_delta: float) -> void:
	var game_node: Game = _find_game_node()
	if not game_node:
		return
	...
	var is_idle: bool = (
		current_ui_state == core.UIState.WAITING and
		current_game_state in [core.GameState.PREPARE, core.GameState.POSTBATTLE]
	)
	
	if (is_idle and 
		current_game_state == _last_game_state and 
		current_ui_state == _last_ui_state):
		_stable_frames += 1
