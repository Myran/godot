...
static func generate_demo_from_session(
	session_id: String, demo_name: String, mode: String = "demo"
) -> Dictionary:
	...
	var metadata: Dictionary = {
		...
		"creation_timestamp":
		Time.get_datetime_string_from_system().replace(":", "").replace("-", "").substr(0, 15),
	...
	var config: Dictionary = SemanticActionMapper.create_replay_config(
		session_id, debug_sequence, metadata, mode
	)
	...
	config["description"] = (
		"Demo from gameplay session: %s (%d actions)" % [session_id, semantic_actions.size()]
	)
...
static func generate_demo_from_latest_session(
	demo_name: String, mode: String = "demo"
) -> Dictionary:
...
static func main() -> void:
	...
	print(
		(
			"📊 Mapping coverage: "
			+ str(report.mapped_actions)
			+ "/"
			+ str(report.total_actions)
			+ " actions mapped"
		)
	)
