...
func execute() -> DebugAction.Result:
	...
	var state_type: String = get_state_type()
	...
	var state_data: Dictionary = capture_data()
	...
	var checksum: String = generate_checksum(state_data)
	...
	return DebugAction.Result.new_success(
		{"checksum": checksum, "state_type": state_type, "action": "captured", "pid": process_id},
...
func _emit_restart_signal(
	checksum: String, state_type: String, process_id: int, test_id: String
) -> void:
	Log.info(
...
static func normalize_data_recursive(data: Variant) -> Dictionary:
	...
	if data is Dictionary:
		...
		for item: Dictionary in DictUtils.get_sorted_items(dict_data):
			var key: Variant = item.key
