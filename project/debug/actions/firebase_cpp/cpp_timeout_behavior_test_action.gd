...
func execute_cpp_action() -> bool:
	...
	operation_tests.append({
		...
		"result": set_result,
		"operation_succeeded": set_worked
	})
	if set_worked: passed_tests += 1
	total_tests += 1
	...
	operation_tests.append({
		...
		"result": get_result,
		"operation_succeeded": get_worked
	})
	if get_worked: passed_tests += 1
	total_tests += 1
	...
	for i in range(3):
		var seq_result = await execute_cpp_operation(
			"set_value_async",
			[["cpp_tests", "basic", "sequential", str(i), str(Time.get_ticks_msec())], "Sequential test " + str(i)],
	...
	operation_tests.append({
		...
		"operation_succeeded": sequential_success
	})
	if sequential_success: passed_tests += 1
	total_tests += 1
	...
	if overall_success:
		_update_status("Basic operations test PASSED (" + str(passed_tests) + "/" + str(total_tests) + " operations succeeded)")
	else:
