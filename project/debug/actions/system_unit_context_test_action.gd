...
func _execute_action_logic(_params: Dictionary = {}) -> DebugAction.Result:
	...
	if tests_passed == total_tests and coverage_percentage >= 100.0:
		return DebugAction.Result.new_success({
			"message": "All UnitContext tests passed with %.1f%% coverage" % coverage_percentage,
			"tests_passed": tests_passed,
			"total_tests": total_tests,
			"coverage_percentage": coverage_percentage,
			"test_data": test_data,
...
func _test_convenience_methods(context: BattleContext, event: Context.Event, results: Array[Dictionary]) -> int:
	...
	var success = (
		self_unit != null and
		self_unit.card_info.get("name", "") == "Allied2" and
		unit_at_0 != null and
		unit_at_0.card_info.get("name", "") == "Allied1" and
		battle_ongoing == true
	)
