...
func _execute_action_logic(_params: Dictionary = {}) -> DebugAction.Result:
	...
	for context_data in contexts:
		...
		benchmark_results[context_name]["deal_damage_to_random_enemies"] = _benchmark_method(
			func(): BattleRules.deal_damage_to_random_enemies(context, true, 5, 2), 1000
		)

		benchmark_results[context_name]["grant_bonuses_to_all_allies"] = _benchmark_method(
			func(): BattleRules.grant_bonuses_to_all_allies(context, 0, true, 1, 1), 1000
		)
	...
	var performance_acceptable = _validate_performance_thresholds(
		benchmark_results, acceptable_thresholds
	)
...
func _create_mock_unit(name: String, health: int, attack: int) -> UnitData:
	...
	unit.card_info = {"name": name, "id": "mock"}
...
func _analyze_performance(benchmark_results: Dictionary) -> Dictionary:
	...
	for method_name in all_methods:
		...
		var avg_across_contexts = (
			times.reduce(func(sum, time): return sum + time, 0.0) / times.size()
		)
		method_averages.append({"method": method_name, "avg_microseconds": avg_across_contexts})
	...
	for method_name in all_methods:
		...
		if times.size() >= 3:  # small, medium, large
			...
			scalability[method_name] = {
				...
				(
					"good"
					if (growth_small_to_medium < 2.0 and growth_medium_to_large < 2.0)
...
func _validate_performance_thresholds(
	benchmark_results: Dictionary, thresholds: Dictionary
) -> bool:
	var position_methods = [
		"get_ally_positions", "get_enemy_positions", "count_allies_alive", "count_enemies_alive"
	]
