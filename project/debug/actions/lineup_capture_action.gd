...
func capture_data() -> Dictionary:
	...
	var allies_lineup = game.holder_allies.get_current_lineup()
	var enemy_lineup = game.holder_enemy.get_current_lineup()
	
	return {
		"allies": extract_lineup_data(allies_lineup),
		"enemy": extract_lineup_data(enemy_lineup)
	}
...
func extract_lineup_data(lineup: Dictionary) -> Dictionary:
	...
	for item in DictUtils.get_sorted_items(lineup):
