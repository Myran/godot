...
func execute_backend_action() -> bool:
	...
	var normal_result = await test_backend_async_pattern("set_data", test_path, test_key, test_value, "Timer Normal")
	...
	for i in range(rapid_requests):
		...
		var rapid_result = await test_backend_async_pattern("set_data", rapid_path, rapid_key, rapid_value, "Rapid " + str(i))
	...
	var test_results = {
		"normal_test": {
			"success": normal_result,
			"duration_ms": normal_duration,
			"within_timeout": normal_ok
		},
		"rapid_test": {
	...
	if overall_success:
		...
		Log.info("Backend TimerManager validation successful", test_results, ["debug", "backend_firebase"])
	else:
		_update_status("Timer Manager test FAILED", true)
		Log.error("Backend TimerManager validation failed", test_results, ["debug", "backend_firebase", "error"])
