...
func _execute_action_logic(_params: Dictionary = {}) -> DebugAction.Result:
	...
	_update_status(
		(
			"BattleRules tests completed: %d/%d passed (%.1f%%)"
	...
	if tests_passed == total_tests and coverage_percentage >= 90.0:
		...
		return DebugAction.Result.new_failure(
			(
				"BattleRules tests failed: %d/%d passed (%.1f%% coverage)"
				% [tests_passed, total_tests, coverage_percentage]
			),
...
func _test_get_enemy_positions(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		allied_enemies == expected_allied_enemies and enemy_enemies == expected_enemy_enemies
	)
	results.append(
		{
...
func _test_get_random_enemy_position(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		allied_enemy_pos in valid_allied_enemies and enemy_enemy_pos in valid_enemy_enemies
	)
	results.append(
		{
...
func _test_get_random_ally_position(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		allied_ally_pos in valid_allied_allies
		and enemy_ally_pos in valid_enemy_allies
		and excluded_allied_pos in valid_excluded
	)
	results.append(
		{
...
func _test_is_position_valid(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	results.append(
		{"test_name": "is_position_valid", "success": all_passed, "test_cases": test_cases}
	)
...
func _test_position_edge_cases(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		empty_positions.size() == 0 and empty_count == 0 and invalid_random == Battle.NO_UNIT_FOUND
	)
