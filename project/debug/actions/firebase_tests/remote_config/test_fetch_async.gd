## Test Remote Config fetch_async method (separate fetch without activate)
class_name TestFetchAsync extends FirebaseTestActionBase

var _remote_config: RemoteConfigService


func _init() -> void:
	super("test.remote_config.fetch_async", _execute_test)
	set_category("Firebase SDK")
	set_group("Remote Config")
	set_description("Test Remote Config fetch async operation (fetch only, no activate)")
...
func _execute_test() -> DebugActionResult:
	...
	# Execute fetch (without activate) and verify result structure
	var result: Variant = await _remote_config.fetch()

	if not assert_not_null(result, "fetch result should not be null"):
		return _assertion_result()

	if not assert_true(result is Dictionary, "fetch result should be a Dictionary"):
		return _assertion_result()
	...
	if result.get("status") == "ok":
		assert_true(true, "fetch completed successfully")
	else:
