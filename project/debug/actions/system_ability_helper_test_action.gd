...
func _execute_action_logic(_params: Dictionary = {}) -> DebugAction.Result:
	...
	_update_status(
		(
			"AbilityHelper tests completed: %d/%d passed (%.1f%%)"
	...
	if tests_passed == total_tests and coverage_percentage >= 100.0:
		...
		return DebugAction.Result.new_failure(
			(
				"AbilityHelper tests failed: %d/%d passed (%.1f%% coverage)"
				% [tests_passed, total_tests, coverage_percentage]
			),
...
func _create_mock_unit(
	name: String, current_health: int, current_attack: int, max_health: int, max_attack: int
) -> UnitData:
...
func _test_event_type_checking(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		damage_pre_correct
		and damage_post_correct
		and damage_pre_false
		and death_correct
		and death_false
		and combat_pre_correct
		and combat_post_correct
	)
...
func _test_event_filtering(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		should_process_damage
		and should_process_death
		and should_not_process_stat
		and null_ability_false
	)
...
func _test_complex_event_combinations(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var is_damage_and_pre = (
		AbilityHelper.is_damage_pre(unit) and not AbilityHelper.is_damage_post(unit)
	)
...
func _test_event_timing_edge_cases(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var pre_matches_pre = (
		AbilityHelper.is_damage_pre(unit_pre) and not AbilityHelper.is_damage_post(unit_pre)
	)
	# POST phase should only match POST methods
	var post_matches_post = (
		AbilityHelper.is_damage_post(unit_post) and not AbilityHelper.is_damage_pre(unit_post)
	)
...
func _test_health_bonus_creation(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	for event in context.unresolved_events:
		if event is BattleContext.StatChangeEvent:
			...
			if (
				stat_event.stat_name == Battle.UNIT_HEALTH
				and stat_event.target_position == 1
				and stat_event.is_allied_side == true
				and stat_event.change_value == 3
			):
				health_events += 1
...
func _test_attack_bonus_creation(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	for event in context.unresolved_events:
		if event is BattleContext.StatChangeEvent:
			var stat_event = event as BattleContext.StatChangeEvent
			if (
				stat_event.stat_name == Battle.UNIT_ATTACK
				and stat_event.target_position == 1
				and stat_event.is_allied_side == true
				and stat_event.change_value == 2
			):
				attack_events += 1
	...
	results.append(
		{"test_name": "attack_bonus_creation", "success": success, "attack_events": attack_events}
	)
...
func _test_damage_creation(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	for event in context.unresolved_events:
		if event is BattleContext.DamageEvent:
			...
			if (
				dmg_event.damage_amount == 4
				and dmg_event.target_position == 1
				and dmg_event.is_allied_side == true
			):
				damage_events += 1
	...
	results.append(
		{"test_name": "damage_creation", "success": success, "damage_events": damage_events}
	)
...
func _test_shield_activation(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	for event in context.unresolved_events:
		if event is BattleContext.ShieldEvent:
			...
			if (
				shield_event.target_position == 1
				and shield_event.is_allied_side == true
				and shield_event.shield_active == true
			):
				shield_events += 1
	...
	results.append(
		{"test_name": "shield_activation", "success": success, "shield_events": shield_events}
	)
...
func _test_custom_stat_bonuses(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	for event in context.unresolved_events:
		if event is BattleContext.StatChangeEvent:
			var stat_event = event as BattleContext.StatChangeEvent
			if stat_event.stat_name == custom_stat and stat_event.change_value == 7:
...
func _test_damage_all_enemies(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	results.append(
		{"test_name": "damage_all_enemies", "success": success, "damage_events": damage_events}
	)
...
func _test_targeting_helpers(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		...
		and target_unit.card_info.get("name", "") == "Allied2"
		and is_from_unit
		and attacker_unit != null
		and attacker_unit.card_info.get("name", "") == "Allied2"
	)
...
func _test_event_data_extraction(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var success = (
		damage_amount == 7
		and stat_change.has("stat_name")
		and stat_change.stat_name == Battle.UNIT_HEALTH
		and stat_change.change_value == 3
	)
...
func _test_counting_with_conditions(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var high_attack_enemies = AbilityHelper.count_enemies_with_condition(
		unit, high_attack_condition
	)
...
func _test_trigger_condition_checking(context: BattleContext, results: Array[Dictionary]) -> int:
	...
	var damage_trigger = AbilityHelper.is_ability_trigger_condition_met(
		damage_unit, "on_take_damage"
	)
	...
	var unknown_trigger = not AbilityHelper.is_ability_trigger_condition_met(
		death_unit, "unknown_trigger"
	)
