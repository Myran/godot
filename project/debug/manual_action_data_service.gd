...
func get_categories() -> Array[String]:
	...
	for category_name in all_categories:
		if not categories.has(category_name) and has_ungrouped_actions(category_name):
			categories.append(category_name)
			Log.debug(
				"Found category with ungrouped actions", {"category": category_name}, [LOG_TAG]
			)

	Log.debug(
		"Retrieved manual action categories",
		{"count": categories.size(), "categories": categories},
		[LOG_TAG]
	)
	return categories
...
func get_groups_for_category(category_name: String) -> Array[String]:
	...
	Log.debug(
		"Retrieved groups for category",
		{"category": category_name, "group_count": groups.size(), "groups": groups},
		[LOG_TAG]
	)
...
func get_ungrouped_actions(category_name: String) -> Array[ManualDebugAction]:
	...
	var ungrouped: Array[ManualDebugAction] = DebugManager.manual_actions.get_ungrouped_actions(
		category_name
	)
	actions.assign(ungrouped)

	Log.debug(
		"Retrieved ungrouped actions",
		{"category": category_name, "action_count": actions.size()},
		[LOG_TAG]
	)
...
func has_ungrouped_actions(category_name: String) -> bool:
	...
	Log.debug(
		"Checked for ungrouped actions",
		{"category": category_name, "has_ungrouped": has_ungrouped},
		[LOG_TAG]
	)
...
func get_all_actions_for_category(category_name: String) -> Dictionary:
	var result: Dictionary = {"grouped": {}, "ungrouped": []}
	...
	Log.debug(
		"Retrieved all actions for category",
		{
...
func get_actions_for_group(category_name: String, group_name: String) -> Array:
	...
	if (
		manual_actions_dict.has(category_name)
		and manual_actions_dict[category_name].has(group_name)
	):
		actions = manual_actions_dict[category_name][group_name].duplicate()

	Log.debug(
		"Retrieved actions for group",
		{"category": category_name, "group": group_name, "action_count": actions.size()},
		[LOG_TAG]
	)
...
func validate_category(category_name: String) -> bool:
	if not _has_manual_actions():
		Log.warning(
			"No manual actions available for validation", {"category": category_name}, [LOG_TAG]
		)
		return false
