help-run:
    ...
    echo ""
    echo "🚨 For debug actions, use 'test-*' commands → just help-debug-modes"
    echo ""
    ...
    echo ""
    echo "🚨 DEBUG ACTION TESTING: See → just help-debug-modes"
    echo ""
...
# Debug Action Execution Mode Help
help-debug-modes:
    #!/usr/bin/env bash
    set -euo pipefail
    echo "🚨 Debug Action Execution Modes Guide"
    echo "====================================="
    echo ""
    echo "CRITICAL DISTINCTION: run-* vs test-* commands for debug actions"
    echo ""
    echo "✅ USE test-* COMMANDS FOR DEBUG ACTIONS:"
    echo "  just test-desktop system-quit-only       # ✅ --test-mode enables debug coordinator"
    echo "  just test-android system-quit-only       # ✅ Debug actions execute properly"
    echo "  just test-desktop-target my-config       # ✅ Final state capture works"
    echo "  just test-android-target my-config       # ✅ Checksum validation active"
    echo ""
    echo "❌ AVOID run-* COMMANDS FOR DEBUG ACTIONS:"
    echo "  just run-desktop                         # ❌ Editor mode, debug coordinator skipped"
    echo "  just run-android-debug                   # ❌ Debug actions won't execute"
    echo ""
    echo "🔍 ROOT CAUSE:"
    echo "  • Desktop runs in editor mode by default"
    echo "  • Debug coordinator automatically skips in editor mode without --test-mode"
    echo "  • test-* commands use --test-mode flag → debug coordinator active"
    echo "  • run-* commands omit --test-mode flag → debug coordinator disabled"
    echo ""
    echo "🎯 AFFECTED FEATURES:"
    echo "  • Final state capture (system.debug.quit_application)"
    echo "  • Checksum validation during replay"
    echo "  • Semantic action logging"
    echo "  • SessionManager integration"
    echo "  • StateExtractor functionality"
    echo ""
    echo "💡 QUICK CHECK:"
    echo "  If debug actions aren't executing → switch from run-* to test-* commands"
