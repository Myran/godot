...
## Test script for JSONPathNavigator and NavigationResult
## Run in the editor to validate functionality
...
func _ready() -> void:
	...
	print("--------------------------------")
	print("JSONPathNavigator Tests Complete")
...
func test_basic_navigation(test_data: Dictionary) -> void:
	...
	var version_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["version"])
	print("Version found: ", version_result.found, " - Value: ", version_result.value)
	
	# Test getting a dictionary
	var settings_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["settings"])
	print("Settings found: ", settings_result.found, " - Is dictionary: ", settings_result.is_dictionary())
	
	# Test getting an array
	var releases_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["releases"])
...
func test_array_navigation(test_data: Dictionary) -> void:
	...
	var player1_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["players", 0])
	...
	var player2_name_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["players", 1, "name"])
	...
	var player_string_index: NavigationResult = JSONPathNavigator.navigate(test_data, ["players", "1", "name"])
...
func test_nested_navigation(test_data: Dictionary) -> void:
	...
	var resolution_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["settings", "graphics", "resolution"])
	print("Resolution found: ", resolution_result.found, " - Value: ", resolution_result.value)
	
	# Test deeply nested array
	var effects_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["settings", "graphics", "effects"])
	print("Effects found: ", effects_result.found, " - Is array: ", effects_result.is_array())
	
	# Test deeply nested array element
	var effect1_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["settings", "graphics", "effects", 0])
	print("First effect found: ", effect1_result.found, " - Value: ", effect1_result.value)
	
	# Test player skills (deeply nested)
	var skills_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["players", 1, "stats", "skills", 2])
...
func test_typed_getters(test_data: Dictionary) -> void:
	...
	var test_data_with_string: Dictionary = {"count": "42"}
	var count: int = JSONPathNavigator.get_int(test_data_with_string, ["count"])
	print("String '42' converted to int: ", count)
	
	# Test type conversion (int to bool)
	var test_data_with_int: Dictionary = {"enabled": 1}
	var enabled: bool = JSONPathNavigator.get_bool(test_data_with_int, ["enabled"])
	print("Int 1 converted to bool: ", enabled)
...
func test_not_found_cases(test_data: Dictionary) -> void:
	...
	var missing_result: NavigationResult = JSONPathNavigator.navigate(test_data, ["missing"])
	print("Missing property found: ", missing_result.found, " - Error: ", missing_result.error_message)
	
	# Test out of bounds array index
	var out_of_bounds: NavigationResult = JSONPathNavigator.navigate(test_data, ["players", 5])
	print("Out of bounds found: ", out_of_bounds.found, " - Error: ", out_of_bounds.error_message)
	
	# Test invalid path type
	var invalid_path: NavigationResult = JSONPathNavigator.navigate(test_data, ["version", "subversion"])
