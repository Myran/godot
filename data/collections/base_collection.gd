...
func get_data_as_dictionary(path: Array[Variant], key: String, use_cache: bool = true) -> Dictionary:
	var data: Variant = await get_data(path, key, use_cache)
	
	if data is Dictionary:
		return data
	
	Log.warning("Data is not a dictionary", {
		"collection": _collection_name,
		"path": path,
...
func get_data_as_array(path: Array[Variant], key: String, use_cache: bool = true) -> Array:
	...
	Log.warning("Data is not an array", {
		"collection": _collection_name,
		"path": path,
		"actual_type": typeof(data)
	}, [Log.TAG_DB, Log.TAG_WARNING])
...
func get_data_as_string(path: Array[Variant], key: String, use_cache: bool = true, default_value: String = "") -> String:
	var data: Variant = await get_data(path, key, use_cache)
	...
	if data != null:
		Log.debug("Converting non-string data to string", {
			"collection": _collection_name,
			"path": path,
			"actual_type": typeof(data)
		}, [Log.TAG_DB])
...
func get_data_as_int(path: Array[Variant], key: String, use_cache: bool = true, default_value: int = 0) -> int:
	var data: Variant = await get_data(path, key, use_cache)
	...
	if data is String:
		...
		if string_value.is_valid_int():
			Log.debug("Converting string to int", {
				"collection": _collection_name,
				"path": path,
				"value": data
			}, [Log.TAG_DB])
...
func save_data(path: Array[Variant], data: Variant) -> bool:
	...
	if success and _cache_enabled:
		...
		_cache[cache_key] = data
		Log.debug("Cache updated after save", {
			"collection": _collection_name,
			"cache_key": cache_key
		}, [Log.TAG_DB, Log.TAG_CACHE])
