...
func get_all(use_cache: bool = true) -> Array[Dictionary]:
	...
	if result == null:
		Log.error("No item data returned", {
			"collection_name": _collection_name,
			"collection_key": _collection_key,
			"base_path": _base_path,
		...
		push_error("Required item data is missing for collection: " + _collection_name + " with key: " + _collection_key)
		...
		Log.warning("Using emergency test items for editor testing ONLY", {
		...
		Log.debug("Processing item data result", {
		...
		if not (result is Array):
			Log.warning("Item data is not an array, attempting to navigate result", {
				"collection_name": _collection_name,
				"result_type": typeof(result),
				"collection_id": get_instance_id()
		...
		if result is Array:
			...
			Log.error("Failed to process item data", {
				"collection_name": _collection_name,
				"result_type": typeof(result),
				"collection_id": get_instance_id()
			}, [Log.TAG_DB, Log.TAG_ERROR])
	...
	if _cache.size() > 0:
		var sample_item: Dictionary = _cache[0]
		var sample_item_keys: Array[String] = sample_item.keys()
		var required_keys: Array[String] = ["id", "name", "type"]
		var missing_keys: Array[String] = []
		
		Log.debug("Validating item data structure", {
		...
		if missing_keys.size() > 0:
			Log.error("Item data missing required fields", {
			...
			push_error("Item data missing required fields: " + str(missing_keys) + 
				". Available keys: " + str(sample_item_keys))
		else:
			Log.debug("Item data validation successful", {
				"collection_name": _collection_name,
				"collection_id": get_instance_id()
			}, [Log.TAG_DB])
	else:
		Log.warning("No items found to validate structure", {
			"collection_name": _collection_name,
			"collection_key": _collection_key,
			"collection_id": get_instance_id()
		}, [Log.TAG_DB, Log.TAG_WARNING])
...
func get_by_type(item_type: String, use_cache: bool = true) -> Array[Dictionary]:
	...
	Log.debug("Found items by type", {"type": item_type, "count": filtered_items.size()}, [Log.TAG_DB])
...
func get_by_rarity(rarity: String, use_cache: bool = true) -> Array[Dictionary]:
	...
	Log.debug("Found items by rarity", {"rarity": rarity, "count": filtered_items.size()}, [Log.TAG_DB])
	return filtered_items
...
func get_by_price_range(min_price: int, max_price: int = -1, use_cache: bool = true) -> Array[Dictionary]:
	Log.info("Getting items by price range", {
		"min_price": min_price, 
		"max_price": max_price,
		"use_cache": use_cache
	}, [Log.TAG_DB])
	...
	Log.debug("Found items in price range", {
		"min_price": min_price, 
		"max_price": max_price,
