...
help-replay:
    #!/usr/bin/env bash
    set -euo pipefail
    echo "🎬 Recording & Replay System with Integrity Testing - Complete User Guide"
    echo "========================================================================"
    echo ""
    echo "📋 OVERVIEW:"
    echo "Comprehensive development tool for capturing semantic player actions, generating test configs,"
    echo "and maintaining system integrity with regression protection. Includes advanced integrity testing"
    echo "to prevent missing component issues and ensure reliable development workflows."
    echo ""
    echo "⚡ QUICK START - Record → Replay → Integrity Validation:"
    echo "  1. just replay-capture-and-generate my-battle-scenario    # Capture logs and generate config"
    echo "  2. just test-android-target my-battle-scenario            # Run the generated replay config"
    echo "  3. just recording-integrity-test                          # Validate entire system integrity"
    echo "  → Config is now part of your test suite with full system validation!"
    echo ""
    ...
    echo "  just replay-capture-and-generate CONFIG_NAME    # Complete workflow: capture → generate → ready for testing"
    echo "  just replay-generate SESSION_ID [CONFIG_NAME]   # Generate replay from specific semantic session"
